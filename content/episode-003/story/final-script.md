...
- Visual intent: 第 0 帧任务气泡已发出，随后浏览器自己打开目标页并开始抓取内容，持续标“功能演示”。
...
- Visual intent: 工作中的浏览器收成数据墙，随后金色八千万和“不是用户数”同时落下。
...
- Visual intent: 先裁切官网任务输入框建立产品入口，随后云电脑里浏览器、文件和进度条同时在动。
...
- Visual intent: 聊天气泡退到背景，随后隔离云电脑里页面滚动、文件落下、命令行光标闪动。
...
- Visual intent: 硬切桌面页标题裁切，随后本机命令弹出并点亮“允许一次”。
...
- Visual intent: 数百万用户叠上开场同一条仍在加载的任务页，随后浏览器继续往下做，持续标“功能演示”。
