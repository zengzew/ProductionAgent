...
能做的事情一多，设置也变复杂。一个人已经写好背景、接上服务，另一个人想用同样的助手，还要重来一遍。Recipe 是 Poke 里的分享功能。它把背景、第一句话和需要连接的服务装进一个链接。别人点开后从这套设置开始，涉及账户时，授权仍要自己完成。

### Narration units

| Text                                                                 | Mode               | Claim IDs                      | Attribution                  |
| -------------------------------------------------------------------- | ------------------ | ------------------------------ | ---------------------------- |
| 能做的事情一多，设置也变复杂。                                       | editorial-analysis | claim-poke-012                 | 由 Recipe 配置项概括设置摩擦 |
| 一个人已经写好背景、接上服务，另一个人想用同样的助手，还要重来一遍。 | demonstration      | claim-poke-012                 | 画面标“功能示意”             |
| Recipe 是 Poke 里的分享功能。                                        | company            | claim-poke-004, claim-poke-012 | 明确 Recipe 属于 Poke        |
| 它把背景、第一句话和需要连接的服务装进一个链接。                     | company            | claim-poke-012                 | 官方产品文档                 |
| 别人点开后从这套设置开始，涉及账户时，授权仍要自己完成。             | company            | claim-poke-012                 | 保留授权边界                 |
...
这些实时动作，也在持续调用模型。创始人举过两个例子：新邮件一到就跑自动化，查航班状态还要不断刷新。任务没结束，推理调用也不会只发生一次。一句很短的请求，背后可能是一串持续运行的调用。他说 Poke 运行昂贵，也很难盈利。随后，Cognition 收购了开发 Poke 的公司。

### Narration units

| Text                                                               | Mode                   | Claim IDs      | Attribution            |
| ------------------------------------------------------------------ | ---------------------- | -------------- | ---------------------- |
| 这些实时动作，也在持续调用模型。                                   | founder                | claim-poke-021 | 承接创始人成本说明     |
| 创始人举过两个例子：新邮件一到就跑自动化，查航班状态还要不断刷新。 | founder                | claim-poke-021 | 不推算单次或总成本     |
| 任务没结束，推理调用也不会只发生一次。                             | editorial-analysis     | claim-poke-021 | 解释实时任务的持续调用 |
| 一句很短的请求，背后可能是一串持续运行的调用。                     | editorial-analysis     | claim-poke-021 | 不推算具体调用次数     |
| 他说 Poke 运行昂贵，也很难盈利。                                   | founder                | claim-poke-021 | 旁白唯一一次创始人归因 |
