...
| `src/lib/pipeline-v2-config.ts`, `pipeline-config.ts`                                                                         | Zod-validated JSON config loader pattern                                                                  | Reused pattern for the six orchestration configs; distinct schemas.                              |
...
| `src/lib/polish.ts` + `config/polish-v2.json` + `prompts/v3/`                                                                 | The one existing hosted-API bounded generate/judge loop; adapter-shape precedent                          | Not the cross-role revision loop; not permission to couple other agents to LangGraph.            |
...
| `src/lib/tts.ts`, `tts-providers.ts` + `config/tts-v2.json`                                                                   | TTS with configured provider + Edge neural fallback (failure-modes §TTS)                                  | Per-segment caching is a target; the no-key/approved-hosted boundary is permanent.               |
...
| `WP-M3-02` | `production-subgraph`: `tts → asset_gen → render → delivery_critic` with L1–L3 rerun edges (no unfreeze). `production-subgraph.ts`.                                                                                                                                                         | `WP-M3-01`, `WP-M2-03` | `delivery-loop.test.ts` → `GOLDEN-003`                                                          | A `delivery.caption-split` reruns the minimum caption→timeline→render→delivery closure; story approval stays valid.                                                                                                                                                     |
| `WP-M3-03` | L4 unfreeze request + human approval + approval-epoch transition + retained stale media. `freeze.ts` (unfreeze half).                                                                                                                                                                       | `WP-M3-02`, `WP-M2-05` | `unfreeze.test.ts` → unfreeze authorization; L1–L4 closure                                      | L4 needs a blocker + human approval + `unfreeze_used < max_unfreeze`; a story/fact issue cannot unfreeze automatically; superseded media is retained for cache reuse.                                                                                                   |
| `WP-M3-04` | Formal `HumanDecision` schema and handlers for both content and final approval gates: approve, reject-as-Issue, direct edit as a new locked artifact version, approval-epoch transitions, and production-stage authorization. `schemas/human-decision`, `freeze.ts`, `graph/main-graph.ts`. | `WP-M1-11`, `WP-M2-07` | `hitl.test.ts` → contract validation plus pause/restart/resume both gates; `REQ-017`, `REQ-020` | M3 replaces no gate location or resume plumbing from M1: it supplies formal payloads and semantics. Reject creates a structured Issue; direct edit creates a hashed locked version; approval advances the epoch; final approval performs no network/upload side effect. |
...
| `WP-M4-02` | Full failure-injection + replay suite. `checkpoint.ts`, `revision.ts`, `agents/`.                                                                      | `WP-M2-05`, `WP-M3-02` | `failure-suite.test.ts` → `FAILURE-001..012`; `replay.test.ts` → `REPLAY-001..004` | Bounded retries, one contract-repair, fake-clock delays, redaction, and event-log rebuild all hold; a tampered log cannot approve an episode.                        |
| `WP-M4-03` | Segment-level TTS cache + shot-level asset cache + cache-key events. `agents/adapters/deterministic-tool.ts`, `observability.ts`.                      | `WP-M3-01`             | `cache.test.ts` → per-segment/per-shot cache keys                                  | Changing one narration segment resynthesizes only that segment; a cache hit is logged with cost 0 and never bypasses aggregate validation.                           |
| `WP-M4-04` | Observability completeness gate + `run-report.md` + derived views. `observability.ts`.                                                                 | `WP-M1-08`             | `observability-complete.test.ts` → completeness/redaction; `REPLAY-003`            | Before approval, every stage has a terminal event and committed checkpoint; usage availability is explicit; `observability-degraded` blocks approval until repaired. |
