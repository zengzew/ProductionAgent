...
| Prompts and configs      | Stable role prompts in `agents/`; Goal 3.2 Oral Rewriter files in `prompts/v4/`; unchanged v3 polish-judge files; polish and TTS JSON configs                                       | There is no orchestration prompt registry or `ownership`, `routing`, `budget`, `models`, or graph-threshold configuration.                                              |
...
| HITL                | Two formal gates: Content Approval before freeze and Final Publication before publish; structured approve/reject/direct-edit actions; dynamic `interrupt()` and `Command(resume=...)`; escalation for budgets, conflicts, compliance, unfreeze, and unrecoverable errors.                                      | **Implemented for WP-M3-04.** `human-decision-v1`, checkpoint-safe interrupts, persisted decision/Issue refs, approval history, epoch-bound production authorization, direct-edit locks, formal unfreeze normalization, and internal-only final approval are executable and tested.               | Remaining work is production-grade persistence/observability hardening and any external product integration; external publishing remains intentionally excluded.                                                                                                                                                                                                       | M-L    | High: an implicit or lost human decision can bypass a required gate.                                 |
