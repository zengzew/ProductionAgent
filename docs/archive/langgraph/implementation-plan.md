...
- Status: **M1–M4 implemented; M4 exit-accepted** (`docs/milestones/m4-acceptance-report.md`). M3 is implemented in tree and covered by HITL/production/unfreeze tests plus the M4 E2E; it has no standalone `docs/m3-acceptance-report.md`.
- Scope: executable LangGraph control plane under `src/orchestration/`; this document remains the work-package authority
- Rewrite date: 2026-08-04; status updated 2026-08-16
...
| `src/lib/project.ts`                                                                                                          | Repo-root guard, legacy CLI episode selection, `readJson`/`writeJson`/`ensureDir`                         | Legacy manual scripts remain outside controlled orchestration; M4.06 runtime identity never uses this process-global selection as concurrent state. |
...
1. `validate:story` used to run `pnpm validate:workflow` without forwarding `--episode`. Fixed in
   `WP-M1-09` (`LEGACY-005`).
...
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------ | -------------------------------------- |
| **M1**    | Deterministic graph skeleton with Stub agents: reference-only state, reducers, artifact registry, two stub `interrupt()` gates, checkpointed pause/resume, one legacy import, validators still green. | —                              | Done                                   |
| **M2**    | Content evaluation and bounded single-owner revision loop: common critic envelope, deterministic route, downstream refresh, Pareto best selection, freeze manifest, locked ranges.                    | M1                             | Done                                   |
| **M3**    | Formal HITL and production: `HumanDecision`, content/final approval semantics, human reject/edit handling, approval epochs, frozen-manifest-only production, L1–L4 repair, idempotent resume.         | M2                             | Done (no standalone acceptance report) |
| **M4**    | Persistence, caching, observability, and migration hardening: production saver, migrations, event completeness, segment/shot caches, full replay/failure suite, full legacy backfill.                 | M3 (parts parallel with M2/M3) | Done — exit-accepted 2026-08-15        |
...
| `WP-M1-09` | **Implemented:** `validate:story` is a single process that forwards `--episode`/`EPISODE_ID` into `validate-workflow`. `package.json`, `scripts/validate-story.ts`.                                                                                                                     | —                                  | `wrapper-episode.test.ts` → `LEGACY-005`                                                                                                 | A non-default episode reaches every nested validator; the test fails if a wrapper validates the default workflow while validating another episode's story.                                                                                                                                                       |
...
| `WP-M4-06` | **Implemented:** multi-episode isolation + per-episode lease/recovery + optimistic checkpoint/artifact CAS + controlled concurrency. `entry.ts`, `checkpoint.ts`, `concurrency.ts`, `artifact-registry.ts`, `observability-gate.ts`. | `WP-M4-01`             | `concurrency.test.ts` → lock, stale recovery, cap, CAS, cross-episode rejection    | Two runs of one episode cannot both advance a revision; different episodes run within the configured cap; reports expose contention and recovery.                                             |
...
`LEGACY-001..005`. `LEGACY-005` is implemented by `WP-M1-09` (`tests/wrapper-episode.test.ts`).
Each work package in §7 names the file that implements its IDs.
