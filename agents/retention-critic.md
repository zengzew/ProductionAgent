...
- `Ending satisfaction`：是否兑现开场承诺，并停在可验证结果。回看同一动作不是
  必须；有来源的收入、融资或估值可以作为兑现。
...
- 前三秒检查的是“结果/动作 + 可理解对象 + 同期证明 + 待回答问题”，不是金额、标题或
  切镜频率。同期证明必须让观众看见动作，品牌首页不能给前三秒满分。
- 执行 `seen-action-not-described-action`：旁白说“自己打开网页去办”，视觉方案却停在
  落地页或同一张官网，必须记为划走风险并退回 Visual Director。中段如果证据形态从不
  切换，也按说明书风险处理。
...
- 结尾必须在 CTA 前回答开场。回看同一动作只在中段赋予新含义时加分。市场预测、
  “你也能赚到”和泛化模型不能替代 payoff；有来源的收入、融资或估值可以。
