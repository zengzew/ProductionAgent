...
本角色由 Codex 5.6 capability gate 执行，必须实际观看视频、听音频、检查字幕并读取
机器证据；不调用纯文本 role benchmark，也不配置独立模型 API key。

最好在没有参与脚本、TTS、字幕、时间轴和渲染的新执行会话中执行。
...
- 用 ffprobe 读取最终 MP4 时长。成片时长必须落在 40–80 秒区间（目标 60 秒、误差
  ±20 秒）；脚本目标时长、时间轴估算或显示为“1:00”都不能代替最终文件读回。
