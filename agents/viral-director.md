...
- `Ending payoff`：结尾如何回答开场问题。优先用有来源的收入、融资、估值或用户
  结果收住；回看同一个动作只在中段改变了它的含义时使用。
...
- 执行 `seen-action-not-described-action`：Hook 的同期证明必须是看见的动作序列
  （手、UI 变化、打开的页面在工作、结果出现），不能指定品牌首页或整页桌面截图作为
  “它自己去办”的唯一画面。`hook-candidates.md` 的首帧结果要写清摄像机看见什么在
  变，而不是产品名叫什么。
...
- 结尾应回答开场问题。若中段改变了开场动作的含义，可以回看；否则停在新的可验证
  结果。有来源、标清口径的收入、融资或估值是资本市场证据，可以作为 payoff。泛化
  财富提问（“你也能赚到”“下一个传奇”）、市场预测和互动 CTA 不算 payoff。

角色示例：前三秒展示“结果 + 一种可追溯证明”，随后问清需求或获客；中段每次回答
都推进那条链，最后用市场给它的价或具体用户结果收住，而不是把第一句再说一遍。
...
- 结尾兑现开场承诺，不加互动 CTA，不升华为行业趋势，也不默认回看开场。
