...
- 优先比较这类结构：有证据的异常结果或动作 → 观众要追的机制问题 → 用户具体痛点与
  产品选择 → 资料支持的受挫、限制或方向变化 → 分发动作与可验证结果 → 回看开场。
  这是一种候选结构，不是每期必须套用的模板。
- 先安排一个观众能看懂的输入、操作、反馈或使用规则，再安排公司历史、平台规模和
  市场判断。抽象结论必须能回到这个用户动作。
- 痛点、团队选择、用户动作、分发方式和结果应成为一条可追踪链。相邻信息只有 Claim
  支持因果时才能标 `causal`；否则明确标 `sequence-only`。
- 转折必须带来新判断。资料没有支持失败、心理低谷或方向变化时，用真实选择、限制或
  结果差异推进，不补危机。
- 结尾回到开场动作或结果时，要让中段的机制与选择改变它的含义；先完成具体 payoff，
  不用行业规律、财富想象或互动问题代替答案。

角色示例：先展示一个已核实但反常的结果，让观众追问“它怎么做到”；随后用产品动作、
团队选择和分发证据逐步回答。只有来源明确支持时，才在中段保留受挫或重新定向。
